...
{       
	...
	std::vector<int> marked_bases(readlen,0);
	int prev_hamming = 0;
	for(auto it = overlap_vec.begin(); it != overlap_vec.end(); ++it)
	{
		if((*it).hamming > prev_hamming)//jump in hamming, try to denoise all marked bases
		{	
			prev_hamming = (*it).hamming;
			for(int i = 0; i < readlen; i++)
			{
				if(marked_bases[i] && !denoised_bases[i])
				{		
					for(int j = 0; j < 4; j++)		
						if((read_count[i][j]+0.25)/(total_count[i]+1) >= param4)
						{
