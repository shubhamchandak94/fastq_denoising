...
std::string outdir;
uint32_t *num_overlapping_reads;
struct overlap 
{
	char *read;
	int shift;
...
int main(int argc, char** argv)
{
	...
	num_overlapping_reads = new uint32_t [numreads];
	...
	std::ofstream fout("read_overlap_count");
	for(int i = 0; i < numreads; i++)
		fout << num_overlapping_reads[i] << "\n";
	delete[] read;
	delete[] quality;
	delete[] read_char;
 	delete[] read_char_RC;
...
void denoise(bitset *read, char(*quality)[readlen+1], std::unordered_map<uint64_t,uint32_t*> *dict)
{
	...
	while(i < stop)
	{
		...
		num_overlapping_reads[i] = overlap_vec.size();
		fout << denoised_read << "\n";
